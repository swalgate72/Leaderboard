...
// Load teams when tournament loads
const _origShowTournDetail = showTournamentDetail;
async function showTournamentDetail(tournamentId) {
  await _origShowTournDetail(tournamentId);
  if (activeTournament?.tournament_type === 'team') {
    activeTournTeams = await tournamentTeamsLoad(tournamentId);
    // Re-render standings with team data
    renderTeamStandings(document.getElementById('td-standings'));
  }
}
