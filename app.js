  ...
  function buildTotals() {
    ...
    sorted.forEach((row, rank) => {
      ...
      t += `<tr style="background:${bg};border-top:1px solid var(--border);">
        ...
        ${showPts  ? `<td style="text-align:center;padding:0.4rem 0.3rem;font-weight:700;color:var(--gold);">${isBest2 && rank === 0 ? (state.groupTotal ?? '') : (row.ptsTotal??'')}</td>` : ''}
      </tr>`;

    // For best2 add a separate team total row
    if (isBest2 && rank === 0) {
      const teamTotal = state.groupTotal ?? 0;
      t += `<tr style="background:rgba(212,168,67,0.1);border-top:2px solid var(--gold);">
        <td style="padding:0.45rem 0.5rem;font-weight:700;color:var(--gold);font-size:0.82rem;" colspan="${isFull18 ? 3 : 1}">
          TEAM TOTAL
        </td>
        ${isFull18 ? '' : ''}
