...
function buildLandscapeScorecard(state, opts = {}) {
  ...
  const playerRows = names.map((name, pi) => {
    ...
    const nets    = log.map(e => {
      if (isStroke)     return e.nets?.[pi]     ?? null;
      if (isStableford) return e.grosses?.[pi] != null ? e.grosses[pi] - (e.extras?.[pi] ?? 0) : null;
      return null;
    });
    const pts = log.map(e => e.holePts?.[pi] ?? e.pts?.[pi] ?? null);
    ...
    // Front/back subtotals (only meaningful for 18 holes)
    ...
    return {
      ...
      front9Gross, back9Gross, front9Pts, back9Pts,
  ...
  sorted.forEach((row, rank) => {
    ...
    // Net shots sub-row (only for stroke/stableford)
    if (showNet && row.nets.some(n => n != null)) {
      ...
      if (isFull18) html += `<td style="background:var(--surface);"></td>`;
      html += `<td style="text-align:center;font-size:0.65rem;color:var(--green);font-weight:600;">${row.netTotal||''}</td>`;
      if (showNet) html += '<td></td>';
      if (showPts) html += '<td></td>';
      html += '</tr>';
    }
