...
// ================================================================
// ----------------------------------------------------------------
// SCORECARD — landscape layout with players as rows, holes as columns
// Used both mid-round (via button) and end-round
// Players ranked: winners at top, losers at bottom
// ----------------------------------------------------------------
function buildLandscapeScorecard(state, opts = {}) {
  ...
  // Sort: Stableford = highest pts first; Stroke = lowest net first; others = highest score first
  const sorted = [...playerRows].sort((a, b) =>
    isStroke ? a.netTotal - b.netTotal : b.score - a.score
  );

  // Column headers
  const holeNums = Array.from({ length: numHoles }, (_, i) => holeOffset + i + 1);
  const parTotals = par;
  ...
  let html = `<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;">
    <table style="border-collapse:collapse;font-size:0.72rem;min-width:600px;width:100%;">
      <thead>
        <tr style="background:var(--surface2);">
          <td style="padding:0.3rem 0.4rem;font-size:0.6rem;color:var(--muted);min-width:80px;"></td>`;

  holeNums.forEach((h, i) => {
    const isF9end = isFull18 && i === 8;
    html += `<td style="text-align:center;padding:0.25rem 0.2rem;font-weight:700;color:var(--muted);font-size:0.65rem;">${h}</td>`;
    if (isF9end) html += `<td style="text-align:center;padding:0.25rem 0.3rem;font-weight:700;color:var(--muted);font-size:0.6rem;background:var(--surface);">Out</td>`;
  });
  if (isFull18) html += `<td style="text-align:center;padding:0.25rem 0.3rem;font-weight:700;color:var(--muted);font-size:0.6rem;background:var(--surface);">In</td>`;
  html += `<td style="text-align:center;padding:0.25rem 0.4rem;font-weight:700;color:var(--gold);font-size:0.65rem;">Total</td>`;
  if (showNet)  html += `<td style="text-align:center;padding:0.25rem 0.3rem;font-weight:700;color:var(--green);font-size:0.6rem;">Net</td>`;
  if (showPts)  html += `<td style="text-align:center;padding:0.25rem 0.3rem;font-weight:700;color:var(--gold);font-size:0.6rem;">Pts</td>`;
  html += '</tr>';

  // Par row
  html += '<tr style="background:var(--surface2);border-bottom:1px solid var(--border);">';
  html += `<td style="padding:0.25rem 0.4rem;font-size:0.6rem;color:var(--muted);">Par</td>`;
  parTotals.forEach((p, i) => {
    const isF9end = isFull18 && i === 8;
    html += `<td style="text-align:center;padding:0.2rem;color:var(--muted);font-size:0.65rem;">${p}</td>`;
    if (isF9end) html += `<td style="text-align:center;padding:0.2rem 0.3rem;color:var(--muted);font-size:0.65rem;background:var(--surface);">${front9Par}</td>`;
  });
  if (isFull18) html += `<td style="text-align:center;padding:0.2rem 0.3rem;color:var(--muted);font-size:0.65rem;background:var(--surface);">${back9Par}</td>`;
  html += `<td style="text-align:center;padding:0.2rem 0.4rem;color:var(--muted);font-weight:700;">${totalPar}</td>`;
  if (showNet)  html += '<td></td>';
  if (showPts)  html += '<td></td>';
  html += '</tr>';

  // SI row
  html += '<tr style="border-bottom:2px solid var(--border);">';
  html += `<td style="padding:0.25rem 0.4rem;font-size:0.6rem;color:var(--muted);">SI</td>`;
  si.forEach((s, i) => {
    const isF9end = isFull18 && i === 8;
    html += `<td style="text-align:center;padding:0.2rem;color:var(--muted);font-size:0.6rem;">${s}</td>`;
    if (isF9end) html += `<td style="background:var(--surface);"></td>`;
  });
  if (isFull18) html += '<td style="background:var(--surface);"></td>';
  html += '<td></td>';
  if (showNet) html += '<td></td>';
  if (showPts) html += '<td></td>';
  html += '</tr>';
  ...
  sorted.forEach((row, rank) => {
    ...
    const rowBg  = isLead ? 'rgba(212,168,67,0.06)' : '';
    const nameCol = pHex(row.pi);

    // Shots row
    html += `<tr style="background:${rowBg};border-top:1px solid var(--border);">`;
    html += `<td style="padding:0.35rem 0.4rem;font-weight:700;color:${nameCol};white-space:nowrap;font-size:0.78rem;">
      ${rank + 1}. ${row.name.split(' ')[0]}
    </td>`;
    row.grosses.forEach((g, i) => {
      const isF9end = isFull18 && i === 8;
      const parV    = par[i] ?? 0;
      const diff    = g != null ? g - parV : null;
      const col     = diff == null ? '' : diff < 0 ? 'var(--green)' : diff > 1 ? 'var(--red)' : '';
      html += `<td style="text-align:center;padding:0.2rem;font-weight:${diff!=null?'600':'400'};color:${col};font-size:0.78rem;">${g ?? ''}</td>`;
      if (isF9end) html += `<td style="text-align:center;padding:0.2rem 0.3rem;font-weight:700;background:var(--surface);font-size:0.75rem;">${row.front9Gross||''}</td>`;
    });
    // Pad remaining holes if round not complete
    for (let i = row.grosses.length; i < numHoles; i++) {
      const isF9end = isFull18 && i === 8;
      html += `<td style="text-align:center;padding:0.2rem;color:var(--muted);font-size:0.65rem;"></td>`;
      if (isF9end) html += `<td style="background:var(--surface);"></td>`;
    }
    if (isFull18) html += `<td style="text-align:center;padding:0.2rem 0.3rem;font-weight:700;background:var(--surface);font-size:0.75rem;">${row.back9Gross||''}</td>`;
    html += `<td style="text-align:center;padding:0.2rem 0.4rem;font-weight:700;color:${nameCol};">${row.grossTotal||''}</td>`;
    if (showNet) html += `<td style="text-align:center;padding:0.2rem 0.3rem;color:var(--green);font-weight:600;">${row.netTotal||''}</td>`;
    if (showPts) html += `<td style="text-align:center;padding:0.2rem 0.3rem;color:var(--gold);font-weight:700;">${row.ptsTotal??''}</td>`;
    html += '</tr>';

    // Net shots sub-row (all formats that have handicap strokes)
    if (showNet && row.nets.some(n => n != null)) {
      html += `<tr style="background:${rowBg};">`;
      html += `<td style="padding:0.1rem 0.4rem;font-size:0.6rem;color:var(--muted);">Net</td>`;
      row.nets.forEach((n, i) => {
        const isF9end = isFull18 && i === 8;
        html += `<td style="text-align:center;padding:0.1rem;color:var(--muted);font-size:0.65rem;">${n??''}</td>`;
        if (isF9end) html += `<td style="text-align:center;background:var(--surface);font-size:0.65rem;color:var(--green);font-weight:600;">${row.front9Net||''}</td>`;
      });
      for (let i = row.nets.length; i < numHoles; i++) {
        const isF9end = isFull18 && i === 8;
        html += '<td></td>';
        if (isF9end) html += `<td style="background:var(--surface);"></td>`;
      }
      if (isFull18) html += `<td style="text-align:center;background:var(--surface);font-size:0.65rem;color:var(--green);font-weight:600;">${row.back9Net||''}</td>`;
      html += `<td style="text-align:center;font-size:0.65rem;color:var(--green);font-weight:700;">${row.netTotal||''}</td>`;
      if (showNet) html += '<td></td>';
      if (showPts) html += '<td></td>';
      html += '</tr>';
    }

    // Points sub-row (stableford only)
    if (showPts && row.pts.some(p => p != null)) {
      html += `<tr style="background:${rowBg};">`;
      html += `<td style="padding:0.1rem 0.4rem;font-size:0.6rem;color:var(--muted);">Pts</td>`;
      row.pts.forEach((p, i) => {
        const isF9end = isFull18 && i === 8;
        html += `<td style="text-align:center;padding:0.1rem;color:var(--gold);font-size:0.65rem;font-weight:600;">${p??''}</td>`;
        if (isF9end) html += `<td style="text-align:center;background:var(--surface);font-size:0.65rem;color:var(--gold);font-weight:600;">${row.front9Pts||''}</td>`;
      });
      for (let i = row.pts.length; i < numHoles; i++) {
        const isF9end = isFull18 && i === 8;
        html += '<td></td>';
        if (isF9end) html += `<td style="background:var(--surface);"></td>`;
      }
      if (isFull18) html += `<td style="text-align:center;background:var(--surface);font-size:0.65rem;color:var(--gold);font-weight:600;">${row.back9Pts||''}</td>`;
      html += `<td style="text-align:center;font-size:0.65rem;color:var(--gold);font-weight:700;">${row.ptsTotal??''}</td>`;
      if (showNet) html += '<td></td>';
      if (showPts) html += '<td></td>';
      html += '</tr>';
    }
